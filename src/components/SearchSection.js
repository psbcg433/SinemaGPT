...
const SearchSection = () => {
  ...
  const handleSearch = () => {
    console.log(`Searching for: ${searchQuery}`, `GPT Search: ${gptSearchEnabled}`);
  };
    ...
    <Box
      sx={{
        ...
        '&::before': {
          ...
          height: '120%',
          background: isDark 
            ? theme.palette.dark?.gradient?.searchGlow || theme.palette.gradient.searchGlow
      ...
      <Container maxWidth="md" disableGutters sx={{ position: 'relative', zIndex: 1 }}>
        ...
        <Box sx={{ textAlign: 'center', mb: 4 }}>
          ...
          <Typography
            variant="h6"
            sx={{
              color: isDark ? theme.palette.dark?.text?.secondary || theme.palette.text.secondary : theme.palette.text.secondary,
              fontWeight: 400,
        ...
        <Paper
          ...
          sx={{
            ...
            background: isDark
              ? `linear-gradient(135deg, ${theme.palette.dark?.background?.paper || '#1E293B'}90, ${theme.palette.dark?.background?.alt || '#334155'}50)`
            ...
            InputProps={{
              startAdornment: (
                <InputAdornment position="start">
                  <Search sx={{ 
                    color: isDark ? theme.palette.dark?.text?.secondary : 'text.secondary',
                    opacity: 0.7 
                  }} />
                </InputAdornment>
              ),
              endAdornment: (
                <InputAdornment position="end" sx={{ mr: 1 }}>
                  <Tooltip title="Enable GPT" arrow disableHoverListener={!isMobile}>
                    <Button
                      ...
                      sx={{
                        ...
                        backgroundColor: gptSearchEnabled
                          ...
                          : isDark ? 'rgba(255,255,255,0.1)' : theme.palette.action.hover,
                        color: gptSearchEnabled 
                          ? theme.palette.common.white 
                          : isDark ? theme.palette.dark?.text?.secondary : theme.palette.text.secondary,
                        '&:hover': {
                          backgroundColor: gptSearchEnabled
                            ? theme.palette.primary.dark
                            : isDark ? 'rgba(255,255,255,0.15)' : theme.palette.action.selected,
                          transform: 'scale(1.05)',
                        },
                        transition: 'all 0.2s ease',
                      }}
                    >
                      {!isMobile && (gptSearchEnabled ? 'GPT On' : 'Enable GPT')}
                    </Button>
                  </Tooltip>
                </InputAdornment>
              ),
